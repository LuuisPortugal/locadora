...
@section('content-view')
    <div class="col-md-9">
        <h3 class="text-muted text-left">
            Lista
            <a href="{{ route("filmeator.create") }}" class="btn btn-success btn-sm pull-right">
            ...
            <div class="table-responsive">
                <table class="table table-condensed table-striped">
                    ...
                    @foreach($filmeAtors as $filmeator)
                        <tr>
                            @foreach($filmeator->toArray() as $campo)
                                <td>
                                    {{ $campo }}
                                </td>
                            @endforeach
                            <td align="center">
                                <form action="{{ route("filmeator.destroy", $filmeator->filmeator_id) }}" method="post">
                                    {{ method_field('DELETE') }}
                                    {{ csrf_field() }}
                                    <a href="{{ route("filmeator.edit", $filmeator->filmeator_id) }}"
